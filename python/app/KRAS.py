...
class KRAS:
	...
	def getResult(self,path,appName,fileName,anotherName):
		...
		if (os.path.exists(report)):
			...
			if(firstLine.startswith('KRAS') and len(firstLines) == 5):
			...
			if(paths[len(paths) - 4] not in PDFPro.userList):
				...
				if(mo.findAllByCondition({'dataKey':paths[len(paths)-2]},'KRASCount').count() > 0):
					mo.deleteAllByCondition({'dataKey':paths[len(paths)-2]},'KRASCount');
				#位点的dict
				resultCount = {};
				if(firstLine.startswith('KRAS') and len(firstLines) == 5):
					#截取userId
