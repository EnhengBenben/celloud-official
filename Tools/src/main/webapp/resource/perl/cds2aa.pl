...
my ($Check,$Trans_table,$Verbose,$Help,$Output);
GetOptions(
...
print OUT "Id\tstart\tstop\tmiddle\ttriple\n" if($Check);
...
while (<>) {
	...
	if ($Check) {
		...
		print OUT "$key\t$start\t$end\t$mid\t$triple\n" if(!$start || !$end || !$mid || !$triple);
		...
		Display_seq(\$prot);
		print OUT ">$head [translate_table: $Trans_table]\n".$prot;
	}
