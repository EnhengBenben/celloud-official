...
function identSim (theDocument) {
	...
	return writeIdentAndSim (titleArray, sequenceArray, arrayOfGroups);
}
...
function writeIdentAndSim (titleArray, sequenceArray, arrayOfGroups)	{
	...
	for (var k = 0; k < sequenceArray.length; k++)	{
		...
		result += '<b>Results for ' + titleArray[k] + ' vs ' + titleArray[m] + ':</b><br/>';
		result += "  Alignment length: " + alignLength + "<br/>";
		result += "Identical residues: " + identical + "<br/>";
		result += "  Similar residues: " + similar + "<br/>";
		if (identical == 0)	{
			result += "  Percent identity: " + 0 + "<br/>";
		}
		else	{
			result += "  Percent identity: " + ((identical/alignLength)*100).toFixed(2) + "<br/>";
		}
		if ((similar == 0) && (identical == 0))	{
			result += "Percent similarity: " + 0 + "<br/>";
		}
		else	{
			result += "Percent similarity: " + (((identical + similar)/alignLength)*100).toFixed(2) + "<br/>";
		}
		result += "<br/>";
		identical = 0;
		similar = 0;
