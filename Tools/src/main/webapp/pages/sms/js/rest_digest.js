...
function restDigest (theDocument) {	
	...
	for (var i = 0; i < restrictionFragments.length; i++)	{
		...
		result += restrictionFragments[i].writeFragment(theDocument.forms[0].elements[4].options[theDocument.forms[0].elements[4].selectedIndex].value);
	}
	return result;
}
...
function writeFragment(topology) {
	...
	if (this.topology == "linear") {
		result += "&gt;" + this.sequence.length + " bp linear fragment from " + topology + " parent " + this.sourceName + ", base " + this.start + " to base " + this.stop + " (" + this.startEnzyme + " - " + this.stopEnzyme + ").<br/>";
	}
	else {
		result += "&gt;" + this.sequence.length + " bp circular molecule from " + topology + " parent " + this.sourceName + ".<br/>";
	}
	result += addReturns(this.sequence) + "<br/><br/>";
	return result;
}
