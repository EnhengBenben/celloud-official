...
function codonPlot (theDocument) {
	...
	result += writeCodonPlot(codonTable, newDna);
	return result;
}
...
function writeCodonPlot(codonTable, sequence) {
	...
	sequence = sequence.replace(/(...)/g,function (str, p1, offset, s) {
		...
		return  "<b>" + p1.toString().toLowerCase() + ", " + (offset + 1) + " to " + (offset + 3) + " (" + aminoAcid + ")</b><br/>" + markString.substring(0, Math.round(yValue * markString.length)) + " " + yValue.toFixed(2) + "<br/><br/>";                     
    });
	return sequence + "<br/>";
}		
