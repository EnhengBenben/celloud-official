...
function cpgIslands (theDocument) {	
	...
	for (var i = 0; i < arrayOfFasta.length; i++)	{
		...
		result += cpgIslandRegions(newDna, 200, 0.6);
	}
	return result;
}
...
function cpgIslandRegions (dnaSequence, windowSize, cutOff)	{
	...
	if (windowSize > dnaSequence.length)	{
		result += "The input sequence must be longer than " + windowSize  + " bases.<br />\n";
		return result;
	}
	...
	if ((valueY >= cutOff) && (gcContent > 0.5))	{
		...
		result += "CpG island detected in region 1 to " + windowSize + " (Obs/Exp = " + valueY + " and %GC = " + gcContent + ")<br />\n";
		islandFound = true;
	}
	...
	for (var j = start; j < dnaSequence.length; j++)	{
		...
		if ((valueY > cutOff) && (gcContent > 0.5))	{
			...
			valueY = valueY.toFixed(2);
			gcContent = gcContent.toFixed(2);
			result += "CpG island detected in region " + startRange + " to " + endRange + " (Obs/Exp = " + valueY + " and %GC = " + gcContent + ") <br />\n";
			islandFound = true;
		}
	}
	if (!(islandFound))	{
		result += "No CpG island regions were identified.<br />\n";
	}
	return result;
}
