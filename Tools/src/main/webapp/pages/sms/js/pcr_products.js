...
function pcrProducts (theDocument) {
	...
	for (var i = 0; i < pcrProducts.length; i++)	{
		result += pcrProducts[i].writeProduct(theDocument.forms[0].elements[8].options[theDocument.forms[0].elements[8].selectedIndex].value);
	}
	if (pcrProducts.length == 0) {
		result += "No PCR products were obtained.<br/><br/>";
	}
	return result;
...
function writeProduct(topology) {
	var result = "";
	result += "&gt;" + this.sequence.length + " bp product from " + topology + " template " + this.template + ", base " + this.start + " to base " + this.stop + " (" + this.forwardName + " - " + this.reverseName + ").<br/>";
