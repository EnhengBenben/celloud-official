...
function rangeExtract (theDocument) {	
	...
	for (var i = 0; i < arrayOfFasta.length; i++)	{
		...
		result += writeSequenceRanges (newDna, ranges, theDocument.forms[0].elements[5].options[theDocument.forms[0].elements[5].selectedIndex].value, theDocument.forms[0].elements[6].options[theDocument.forms[0].elements[6].selectedIndex].value);
	}
	return result;
}
...
function writeRanges(sequence) {
	...
	if (this.type == "new_sequence") {
		...
		if (this.strand == "reverse") {
		        result += addReturns(reverse(complement(sequenceArray.join("")))) + "<br/><br/>";
	        }
		else {
	...
	if (this.type == "separate") {
		for (var i = 0; i < this.ranges.length; i++) {
			if (this.ranges[i].getSequence(sequence) != "") {
				...
				if (this.strand == "reverse") {
					result += reverse(complement(addReturns(this.ranges[i].getSequence(sequence)))) + "<br/><br/>";				        
				}
				else {
	...
	if (this.type == "uppercased") {
	    ...
	    if (this.strand == "reverse") {
	    	result += reverse(complement(addReturns(sequence))) + "<br/><br/>";
		}
		else {
	...
	if (this.type == "preserved") {
		...
		if (this.strand == "reverse") {
			result += reverse(complement(addReturns(randomSequence))) + "<br/><br/>";
		}
		else {
