...
public class CompanyDaoImpl implements CompanyDao {
	...
	public List<Company> getCompanyDetailById(Integer companyId) {
		log.info("获取医院详细信息");
		String sql = "select c.company_id,c.company_name,c.address,c.tel,c.create_date,count(u.username) userNum,(select count(f.file_name) from tb_file f where f.user_id=u.user_id and f.state=0) fileNum,(select ifnull(sum(f.size),0) from tb_file f where f.user_id=u.user_id and f.state=0) fileSize,(select count(*) from tb_report r where r.user_id=u.user_id and r.isdel=0 and (r.flag=0 or r.report_id=11)) reportNum from tb_company c,tb_dept d,tb_user u where c.company_id=d.company_id and d.dept_id=u.dept_id and c.state=0 and u.state=0 and u.company_id=? and u.user_id not in ("
				+ noUserid + ") group by c.company_id order by fileNum desc;";
