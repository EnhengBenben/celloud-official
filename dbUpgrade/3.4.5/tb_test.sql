...
INSERT INTO `tb_app_tag_relat` (`app_id`, `tag_id`) SELECT (SELECT max(app_id) FROM tb_app), atr.tag_id FROM tb_app_tag_relat atr WHERE atr.app_id = 82;
UPDATE tb_app a1 SET a1.app_name = '桑格测序数据分析', a1.english_name ='桑格测序数据分析', a1.company_id = 1 WHERE a1.app_id = (SELECT a3.maxId FROM (SELECT max(a2.app_id) maxId FROM tb_app a2) a3);
...
INSERT INTO `tb_app_tag_relat` (`app_id`, `tag_id`) SELECT (SELECT max(app_id) FROM tb_app), atr.tag_id FROM tb_app_tag_relat atr WHERE atr.app_id = 114;
UPDATE tb_app a1 SET a1.app_name = '高通量测序数据分析', a1.english_name ='高通量测序数据分析', a1.company_id = 1 WHERE a1.app_id = (SELECT a3.maxId FROM (SELECT max(a2.app_id) maxId FROM tb_app a2) a3);
