...
function revTrans (theDocument) {	
	...
	for (var i = 0; i < arrayOfFasta.length; i++)	{
		...
		result += writeRevTransSeqNoDegen(newProtein, title, codonTable);
		result += "<br/>";
		result += writeRevTransSeqDegen(newProtein, title, codonTable);
		result += "<br/>";
		result += "Graph of base probabilities:<br/>";		
		result += writeRevTransGraph(newProtein, codonTable);
...
function writeRevTransSeqNoDegen(protein, title, codonTable) {
	...
	result += '&gt;' + 'reverse translation of ' + title + ' to a ' + protein.length + ' base sequence of most likely codons.<br/>';
	result += addReturns(protein);
	result += "<br/>";
	return result;
}
...
function writeRevTransSeqDegen(protein, title, codonTable) {
	...
	result += '&gt;' + 'reverse translation of ' + title + ' to a ' + protein.length + ' base sequence of consensus codons.<br/>';
	result += addReturns(protein);
	result += "<br/>";
	return result;
}
...
function writeRevTransGraph(protein, codonTable) {
	...
	protein = protein.replace(/(.)/g,
                    function (str, p1, offset, s) {
			aminoAcid = codonTable[p1.toString().toLowerCase()];
			return "<b>" + (offset + 1) + "_" + str + "_" + "first</b><br/>" + aminoAcid.rulerPosOne + "<b>" + (offset + 1) + "_" + str + "_" + "second</b><br/>" + aminoAcid.rulerPosTwo + "<b>" + (offset + 1) + "_" + str + "_" + "third</b><br/>" + aminoAcid.rulerPosThree + "<br/>";                      
                   }
                );
...
function getRuler(freq, mark) {
	return mark.substring(0, 1) + mark.substring(0, freq * mark.length) + " " + (freq).toFixed(2) + "<br/>";	
}
