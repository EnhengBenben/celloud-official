...
function mutateForDigest (theDocument) {
	...
	result += "<span class=\"mutated_sequence\">" + "sequence and translations for mutated version" + "</span><br/>";
	result += "<span class=\"current_sequence\">" + "sequence and translations for non-mutated version" + "</span><br/>";
	result += "<br/>";			
	...
	for (var i = 0; i < arrayOfFasta.length; i++)	{
		...
		result += getInfoFromTitleAndSequenceAndTopology(title, newDna, TOPOLOGY);
		...
		result += layoutRestTrans (newDna, mutatedDna, geneticCode, restrictionSiteCollection, mutatedRestrictionSiteCollection, theDocument.forms[0].elements[5].options[theDocument.forms[0].elements[5].selectedIndex].value, theDocument.forms[0].elements[6].options[theDocument.forms[0].elements[6].selectedIndex].value);
		result += '<br/><br/>';
	}
	return result;
}	
...
function layoutRestTrans (dnaSequence, mutatedDnaSequence, geneticCode, restrictionSiteCollection, mutatedRestrictionSiteCollection, basesPerLine, readingFrame)	{
	...
	for (var i = 0; i < dnaSequence.length; i = i + basesPerLine) {
		...
		for (var j = 0; j < sitesInRangeMut.length; j++) {
			...
			result += spaceString.substring(0, sitesInRangeMut[j].position - i + 9) + "<span class=\"mutated_sequence\">" + sitesInRangeMut[j].label + "</span><br/>";
		}

		for (var j = 0; j < sitesInRange.length; j++) {
			//9 is added because of indent to dna sequence
			result += spaceString.substring(0, sitesInRange[j].position - i + 9) + "<span class=\"current_sequence\">" + sitesInRange[j].label + "</span><br/>";
		}
...
TranslationComponent.prototype.writeLayoutComponent = function (start, stop) {
	...
	result += this.spanStart;
	result += this.characters.slice(start, stop).join("");
	result += this.spanEnd + "<br/>";
