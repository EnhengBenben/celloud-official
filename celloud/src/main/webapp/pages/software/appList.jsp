...
<div class="y-row clearfix">
  ...
  <div id="JS_listBox0" class="tabBox">
    <ul data-spm="201">
      <c:if test="${!empty sclassifys}">
	    <li s-key="deliveryMethod" class="J_single" data-spm="2">
	      <div class="list-left">分类详细：</div>
	      <div class="list-right">
	        <a key-v="all" href="javascript:toMoreApp(${classifyPid },0,1,0)" <c:if test='${classifyId==0 }'>class="current"</c:if> id="sid0">全部</a>
	        <c:forEach items="${sclassifys }" var="sc">
	          <a key-v="${sc.classifyName }" <c:if test='${sc.classifyId==classifyId }'>class="current"</c:if> href="javascript:toMoreApp(${sc.classifyPid },${sc.classifyId },1,1)" id="sid${sc.classifyId }">${sc.classifyName }</a>
	        </c:forEach>
	      </div>
	    </li>
      </c:if>
      <li s-key="priceTag" class="J_single" data-spm="1">
        <div class="list-left">价格：</div>
        <div class="list-right">
          <a class="current" key-v="all" target="_self" href="#">全部</a>
          <a target="_self" href="#" key-v="0-0">免费</a>
        </div>
      </li>
	</ul>
  </div>
  <ul class="sort-list" data-spm="202" id="sort-listUl">
    <li class="current" id="defaultSort">
      <a d-order="" a-order="" order="" href="javascript:toMoreApp(${classifyPid },${classifyId },1,${classifyFloor });" id="defaultSort">默认排序</a>
    </li>
    <li class="" data-spm="1" id="sortByCreateDate">
      <a d-order="created_on-desc" a-order="created_on-asc" target="_self" href="javascript:void();" id="sortByCreateDate">上线时间<i class="down"></i></a>
    </li>
<!--     <li class="" data-spm="2"> -->
...
<!-- 		               </div> -->
		               <span class="score">${app.createDate }</span>
		            </div>
				  ...
				  <c:otherwise>
					  ...
					  <c:otherwise>
					    <c:if test="${appPageList.page.currentPage>4 && (appPageList.page.totalPage-appPageList.page.currentPage>6)}">
							<li><a href="javascript:toMoreApp(${classifyPid },${classifyId },<s:property value="appPageList.page.currentPage+6"/>,${classifyFloor })"><s:property value="appPageList.page.currentPage+6"/></a></li>
						</c:if>
					  </c:otherwise>
					</c:choose>
				  </c:otherwise>
