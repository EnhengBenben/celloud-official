	      	...
	      	<c:choose>
		      	<c:when test="${fn:length(fileName)>2}">
					<c:forEach items="${fileName }" var="fname" begin="0" end="1" varStatus="status">
						<c:set var="name" value="${name.concat(fname) }"></c:set><c:if test="${status.count == 1}"><c:set var="name" value="${name.concat('_') }"></c:set> </c:if>
					</c:forEach>
		      	</c:when>
		      	<c:otherwise>
		      		<c:set var="name" value="${fn:substringBefore(pgs.fileName, '.') }"></c:set>
		      	</c:otherwise>
	      	</c:choose>
	      	${name.length() > 16 ? name.substring(0,11).concat('...') : name }
	      </td>
	      <td align="center">${pgs.anotherName.length() > 14 ? (pgs.anotherName.substring(0,11).concat('...')) : pgs.anotherName }</td>
