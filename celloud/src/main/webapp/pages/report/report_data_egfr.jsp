	   ...
	   <dl class="dl-horizontal datareport-title">
        ...
        <div class="toolbar">
            ...
            <c:if test="${egfr.pdf!=null && egfr.pdf!='' }">
                <a class="btn btn-warning btn-flat" href="/report/down?path=${egfr.appId }/${egfr.dataKey }/${egfr.pdf }"><i class="fa fa-file-pdf-o"></i>PDF下载</a>
            </c:if>
        </div>
	</div>
	<!--位点突变-->
	    ...
	    <div class="m-boxCon result">
	    	...
	    	<c:if test="${egfr.knowMutationBig!=null }">
		    	<c:forEach var="kmb" items="${egfr.knowMutationBig }">
		    		<br/>
					<img name="know" src="${uploadPath}${egfr.userId }/${egfr.appId }/${egfr.dataKey }/SVG/${kmb }" style="width: 100%;">
		    	</c:forEach>
			</c:if>
	    </div>
	</div>
