(function() {
	celloudApp.controller("fileUpload", function($scope,uploadService) {
		$scope.step = 'one';
		$scope.nextStep = function(){
			$scope.step = 'two';
		}
		$scope.tags = uploadService.getProductTags().query();
...
//		      handleStatus(uploader.total.percent);
		    ...
		    uploader.bind("FilesAdded", function(uploader, files) {
		      ...
		      $.each(files, function(index, item) {
		        var $fileDom_uploading = $('<tr id="uploading-' + item.id + '">');
		        ...
		        $fileDom_uploading.append($('<td class="plupload-file-action"><a href="#" style="display: block;"><i class="fa fa-times-circle-o" aria-hidden="true"></i></a></div>'));
		        $fileDom_uploading.append($('<div class="plupload-clearer"></td>&nbsp;</tr>'));
		        $("#upload-list-tbody").append($fileDom_uploading);
		        handleStatus(item);
...
//		      handleStatus(file);
		    ...
		    uploader.bind("BeforeUpload", function(uploader, file) {
		       uploader.setOption("multipart_params",{'originalName': file.name,'tagId':$scope.tagId,'batch': $scope.batch});
		    });
		...
		var upload = initUploader(["choseFile","uploadMore"]);
		upload.init();
	});
})();
