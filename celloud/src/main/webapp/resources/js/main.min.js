...
!function(){celloudApp.service("bsiService",function($http){this.reportPageQuery=function(params){return $http({method:"POST",url:"report/bsi/reportPageQuery",data:$.param(params),headers:{"Content-Type":"application/x-www-form-urlencoded"}})},this.reportReRun=function(dataKey,appId,projectId){return $http({method:"POST",url:"data/reRun",data:$.param({dataKey:dataKey,appId:appId,projectId:projectId}),headers:{"Content-Type":"application/x-www-form-urlencoded"}})},this.getBSIPatientReport=function(params){return $http({method:"POST",url:"report/getBSIPatientReportInfo",data:$.param(params),headers:{"Content-Type":"application/x-www-form-urlencoded"}})},this.dataPageQuery=function(params){return $http({method:"POST",url:"data/bsi/dataPageQuery",data:$.param(params),headers:{"Content-Type":"application/x-www-form-urlencoded"}})}})}();
!function(){celloudApp.controller("bsiCommon",function($scope,$rootScope){function getSize(fileSize){var unit="b";return fileSize>1e3&&(fileSize/=1e3,unit="kb"),fileSize>1e3&&(fileSize/=1e3,unit="mb"),fileSize.toFixed(2)+unit}function handleStatus(file){var actionClass,iconClass;file.status==plupload.DONE&&(actionClass="plupload_done",iconClass="fa fa-check-circle"),file.status==plupload.FAILED&&(actionClass="plupload_failed",iconClass="fa fa-exclamation-triangle"),file.status!=plupload.QUEUED&&file.status!=plupload.UPLOADING||(actionClass="plupload_delete",iconClass="fa fa-times-circle-o");var icon=$("#uploading-"+file.id).attr("class",actionClass).find("i").attr("class",iconClass);file.hint&&icon.attr("title",file.hint)}$rootScope.sortIcon=function(params){"asc"==params.sortDate?$("#sort-date-icon").removeClass("fa-sort-amount-desc").addClass("fa-sort-amount-asc"):$("#sort-date-icon").removeClass("fa-sort-amount-asc").addClass("fa-sort-amount-desc"),"asc"==params.sortBatch?$("#sort-batch-icon").removeClass("fa-sort-amount-desc").addClass("fa-sort-amount-asc"):$("#sort-batch-icon").removeClass("fa-sort-amount-asc").addClass("fa-sort-amount-desc"),"asc"==params.sortName?$("#sort-name-icon").removeClass("fa-sort-amount-desc").addClass("fa-sort-amount-asc"):$("#sort-name-icon").removeClass("fa-sort-amount-asc").addClass("fa-sort-amount-desc"),"asc"==params.sortPeriod?$("#sort-period-icon").removeClass("fa-sort-amount-desc").addClass("fa-sort-amount-asc"):$("#sort-period-icon").removeClass("fa-sort-amount-asc").addClass("fa-sort-amount-desc")},$scope.uploadPercent=0,$scope.itemBtnToggleActive=function(){$("#common-menu .item-btn").removeClass("active"),$("#to-upload-a").addClass("active")},$scope.stepOne=function(){$scope.itemBtnToggleActive(),$rootScope.bsiUploader&&"two"==$rootScope.bsiStep?($scope.stepTwo(),$("#upload-filelist").children().remove(),$("#uploading-filelist").children(":not(:first)").remove(),$.each($rootScope.bsiUploader.files,function(index,item){var $fileDom=$('<li class="plupload_delete" id="'+item.id+'">');$fileDom.append($('<div class="plupload-file-name"><span>'+item.name+"</span></div>")),$fileDom.append($('<div class="plupload-file-size">'+getSize(item.size)+"</div>")),$fileDom.append($('<div class="plupload-file-action"><a href="#" style="display: block;"><i class="fa fa-times-circle-o" aria-hidden="true"></i></a></div>')),$fileDom.append($('<div class="plupload-clearer"></div>&nbsp;</li>')),$("#upload-filelist").append($fileDom),handleStatus(item);var params={size:item.size,lastModifiedDate:item.lastModifiedDate,name:item.name};null!=box&&$.get(box+"/box/checkBreakpoints",params,function(data){data.data&&(item.loaded=data.data,$("#uploading-"+item.id+" .plupload-file-status").html(100*(item.loaded/item.size).toFixed(2)+"%"))}),$("#"+item.id+".plupload_delete a").click(function(e){$("#"+item.id).remove(),$("#uploading-"+item.id).remove(),$rootScope.bsiUploader.removeFile(item),e.preventDefault(),utils.stopBubble(e),$scope.uploadTextType()}),$("#uploading-"+item.id+".plupload_delete a").click(function(e){$("#"+item.id).remove(),$("#uploading-"+item.id).remove(),$rootScope.bsiUploader.removeFile(item),e.preventDefault(),utils.stopBubble(e)})})):$rootScope.bsiUploader&&"three"==$rootScope.bsiStep?($scope.beginUpload(),$("#upload-filelist").children().remove(),$("#uploading-filelist").children(":not(:first)").remove(),$.each($rootScope.bsiUploader.files,function(index,item){var $fileDom_uploading=$('<li id="uploading-'+item.id+'">');$fileDom_uploading.append($('<div class="plupload-file-name"><span title="'+item.name+'">'+item.name+"</span></div>")),$fileDom_uploading.append($('<div class="plupload-file-size">'+getSize(item.size)+"</div>")),$fileDom_uploading.append($('<div class="plupload-file-surplus">_</div>')),$fileDom_uploading.append($('<div class="plupload-file-status">_</div>')),$fileDom_uploading.append($('<div class="plupload-file-action"><a href="#" style="display: block;"><i class="fa fa-times-circle-o" aria-hidden="true"></i></a></div>')),$fileDom_uploading.append($('<div class="plupload-clearer"></div>&nbsp;</li>')),$("#uploading-filelist").append($fileDom_uploading),handleStatus(item);var params={size:item.size,lastModifiedDate:item.lastModifiedDate,name:item.name};null!=box&&$.get(box+"/box/checkBreakpoints",params,function(data){data.data&&(item.loaded=data.data,$("#uploading-"+item.id+" .plupload-file-status").html(100*(item.loaded/item.size).toFixed(2)+"%"))}),$("#"+item.id+".plupload_delete a").click(function(e){$("#"+item.id).remove(),$("#uploading-"+item.id).remove(),$rootScope.bsiUploader.removeFile(item),e.preventDefault(),utils.stopBubble(e),$scope.uploadTextType()}),$("#uploading-"+item.id+".plupload_delete a").click(function(e){$("#"+item.id).remove(),$("#uploading-"+item.id).remove(),$rootScope.bsiUploader.removeFile(item),e.preventDefault(),utils.stopBubble(e)})})):($rootScope.bsiStep="one",$(".plupload_filelist li").hasClass("plupload_droptext")&&($("#batch-info").val(""),$("#batch-div").removeClass("hide"),$("#upload-content").addClass("upload-step-one")),$scope.uploadPercent>=100&&(waveLoading.setProgress(0),document.querySelector("#upload-progress").height=document.querySelector("#upload-progress").height)),$("#upload-modal").modal("show")},$scope.uploadTextType=function(){$("#uploading-filelist").children().length>2?$(".upload-text").addClass("hide"):$(".upload-text").removeClass("hide")},$scope.stepTwo=function(){$(".step-one-content").addClass("hide"),$(".step-two-content").removeClass("hide"),$("#one-to-two").addClass("active"),$(".step-two").addClass("active"),$scope.uploadTextType(),"one"==$rootScope.bsiStep&&initUploader()},$scope.batchInfo=function($event){13==$event.keyCode&&$scope.stepTwo()},$scope.refreshSession=function(){$.get("uploadFile/checkAdminSessionTimeOut")};var initUploader=function(){$.get(CONTEXT_PATH+"/box/configs",function(configs){var box=null;for(var index in configs){var config=configs[index],port=config.port||80,context=config.context?config.context.startsWith("/")?config.context:"/"+config.context:"";config="http://"+config.intranetAddress+":"+port+context;var response=$.ajax(config+"/box/alive",{async:!1}).responseText;if(response&&JSON.parse(response).success){box=config;break}}var uploadUrl=null==box?"../uploadFile/uploadManyFile":box+"/box/upload";console.log(null==box?"没有找到盒子...":"成功找到了一个盒子，地址为："+box);var uploader=new plupload.Uploader({runtimes:"html5,flash,silverlight,html4",browse_button:["plupload-content","upload-more"],url:uploadUrl,chunk_size:"1mb",dragdrop:!0,unique_names:!0,drop_element:"upload-filelist",filters:{max_file_size:"10gb",prevent_duplicates:!0,mime_types:[{title:"bam",extensions:"bam"},{title:"ab1",extensions:"ab1"},{title:"abi",extensions:"abi"},{title:"fasta",extensions:"fasta"},{title:"fastq",extensions:"fastq"},{title:"tsv",extensions:"tsv"},{title:"gz",extensions:"gz"},{title:"lis",extensions:"lis"},{title:"txt",extensions:"txt"}]},max_retries:5,multiple_queues:!0,flash_swf_url:"//cdn.bootcss.com/plupload/2.1.8/Moxie.swf"});uploader.init(),uploader.bind("StateChanged",function(){uploader.state===plupload.STARTED?(window.parent.isUploading=!0,refresh=setInterval("self.refreshSession()",6e5)):uploader.state===plupload.STOPPED&&(window.parent.isUploading=!1,clearInterval(refresh))}),uploader.bind("UploadProgress",function(uploader,file){waveLoading.setProgress(uploader.total.percent),uploadPercent=uploader.total.percent,$("#uploading-"+file.id+" .plupload-file-status").html(file.percent+"%"),$("#uploading-"+file.id+" .plupload-file-surplus").html(utils.formatDate((file.size-file.loaded)/uploader.total.bytesPerSec)),handleStatus(uploader.total.percent)}),uploader.bind("FilesAdded",function(uploader,files){$.get("uploadFile/checkAdminSessionTimeOut",function(response){response||uploader.destroy()}),$.each(files,function(index,item){var $fileDom=$('<li class="plupload_delete" id="'+item.id+'">');$fileDom.append($('<div class="plupload-file-name"><span>'+item.name+"</span></div>")),$fileDom.append($('<div class="plupload-file-size">'+getSize(item.size)+"</div>")),$fileDom.append($('<div class="plupload-file-action"><a href="#" style="display: block;"><i class="fa fa-times-circle-o" aria-hidden="true"></i></a></div>')),$fileDom.append($('<div class="plupload-clearer"></div>&nbsp;</li>')),$("#upload-filelist").append($fileDom);var $fileDom_uploading=$('<li id="uploading-'+item.id+'">');$fileDom_uploading.append($('<div class="plupload-file-name"><span title="'+item.name+'">'+item.name+"</span></div>")),$fileDom_uploading.append($('<div class="plupload-file-size">'+getSize(item.size)+"</div>")),$fileDom_uploading.append($('<div class="plupload-file-surplus">_</div>')),$fileDom_uploading.append($('<div class="plupload-file-status">_</div>')),$fileDom_uploading.append($('<div class="plupload-file-action"><a href="#" style="display: block;"><i class="fa fa-times-circle-o" aria-hidden="true"></i></a></div>')),$fileDom_uploading.append($('<div class="plupload-clearer"></div>&nbsp;</li>')),$("#uploading-filelist").append($fileDom_uploading),handleStatus(item);var params={size:item.size,lastModifiedDate:item.lastModifiedDate,name:item.name};null!=box&&$.get(box+"/box/checkBreakpoints",params,function(data){data.data&&(item.loaded=data.data,$("#uploading-"+item.id+" .plupload-file-status").html(100*(item.loaded/item.size).toFixed(2)+"%"))}),$("#"+item.id+".plupload_delete a").click(function(e){$("#"+item.id).remove(),$("#uploading-"+item.id).remove(),uploader.removeFile(item),e.preventDefault(),utils.stopBubble(e),$.upload.uploadTextType()}),$("#uploading-"+item.id+".plupload_delete a").click(function(e){$("#"+item.id).remove(),$("#uploading-"+item.id).remove(),uploader.removeFile(item),e.preventDefault(),utils.stopBubble(e)})}),$rootScope.bsiStep="two",$scope.uploadTextType()}),uploader.bind("FileUploaded",function(uploader,file,response){response.response;handleStatus(file)}),uploader.bind("UploadComplete",function(uploader,files){files.length>0?(waveLoading.setProgress(100),uploadPercent=100):(waveLoading.init({haveInited:!1}),waveLoading.draw(),document.querySelector("#upload-progress").height=document.querySelector("#upload-progress").height),$(".step-one-content").removeClass("hide"),$(".step-two-content").addClass("hide"),$(".step-three-content").addClass("hide"),$("#one-to-two").removeClass("active"),$("#two-to-three").removeClass("active"),$(".step-two").removeClass("active"),$(".step-three").removeClass("active"),$("#batch-info").val(""),uploader.splice(),$rootScope.bsiStep="one",$rootScope.bsiUploader.destroy(),$rootScope.bsiUploader=void 0,$("#uploading-filelist .plupload_done").remove(),$("#upload-modal").modal("hide")}),uploader.bind("BeforeUpload",function(uploader,file){uploader.setOption("multipart_params",{userId:window.userId,lastModifiedDate:file.lastModifiedDate,size:file.size,originalName:file.name,name:file.name,tagId:$("#tag-info").val(),batch:$("#batch-info").val(),needSplit:$("#need-split:checked").val()})}),uploader.bind("Error",function(uploader,error){"-602"==error.code&&alert("当前队列已存在文件【"+error.file.name+"】，请勿重复添加！")}),$rootScope.bsiUploader=uploader})};$scope.beginUpload=function(){$(".step-three-content").removeClass("hide"),$(".step-one-content").addClass("hide"),$(".step-two-content").addClass("hide"),$("#two-to-three").addClass("active"),$(".step-two").addClass("active"),$(".step-three").addClass("active"),$("#tags-review").html($("#batch-info").val()),$rootScope.bsiUploader.files.length>0&&"three"!=$rootScope.bsiStep&&($("#upload-filelist").html(""),$rootScope.bsiUploader.start(),$rootScope.bsiStep="three",waveLoading.init({haveInited:!0,target:document.querySelector("#upload-progress"),color:"rgba(40, 230, 200, 0.6)",showText:!1}),waveLoading.draw(),waveLoading.setProgress(0))},$scope.closeUploadModal=function(){$rootScope.bsiUploader.files.length<=0&&($(".step-one-content").removeClass("hide"),$(".step-two-content").addClass("hide"),$("#one-to-two").removeClass("active"),$(".step-two").removeClass("active"),$(".step-three-content").addClass("hide"),$("#two-to-three").removeClass("active"),$(".step-three").removeClass("active"));var text=$("body .breadcrumb").text();text.indexOf("数据")>=0?$scope.itemBtnToggleActive($("#to-data-a")):text.indexOf("报告")>=0&&$scope.itemBtnToggleActive($("#to-report-a"))}}),celloudApp.controller("bsiReportDataController",function($scope,$rootScope,$routeParams,bsiService){var params={reportIndex:$routeParams.reportIndex,dataKey:$routeParams.dataKey,projectId:$routeParams.projectId,appId:$routeParams.appId,page:$rootScope.bsiReportParams.page,condition:$rootScope.bsiReportParams.condition,sort:$rootScope.bsiReportParams.sort,sortDate:$rootScope.bsiReportParams.sortDate,sortPeriod:$rootScope.bsiReportParams.sortPeriod,sortBatch:$rootScope.bsiReportParams.sortBatch,sortName:$rootScope.bsiReportParams.sortName,size:$rootScope.bsiReportParams.size};bsiService.getBSIPatientReport(params).success(function(dataMap){$scope.bsiCharList=dataMap.bsiCharList,$scope.bsi=dataMap.bsi,$scope.pageList=dataMap.pageList,$scope.data=$scope.data,$scope.tab="patient";for(var havestrain="",i=0;i<$scope.bsi.species_20.length;i++)havestrain+=$scope.bsi.species_20[i].species_zh+",";$scope.havestrain=havestrain.substr(0,havestrain.length-1),$scope.getRowspan=function(val1,val2,val3){var val0=1;return null!=val1&&val0++,null!=val2&&val0++,null!=val3&&val0++,val0}})}),celloudApp.controller("bsiReportController",function($scope,$rootScope,bsiService){$rootScope.bsiReportParams={page:1,size:20,condition:null,sort:0,sortBatch:"asc",sortName:"asc",sortPeriod:"asc",sortDate:"desc",reportType:0,batch:null,period:null,beginDate:null,endDate:null,distributed:null,sampleName:null},$scope.pageQuery=function(){bsiService.reportPageQuery($rootScope.bsiReportParams).success(function(dataMap){$scope.batchList=dataMap.batchList,$scope.pageList=dataMap.pageList,$scope.periodMap=dataMap.periodMap,$scope.nowDate=dataMap.nowDate})},$scope.reportBatchSearch=function(batchId){$("#batch-sl").hasClass("select-more")||($rootScope.bsiReportParams.batch="'"+$("#"+batchId).next().find("span").text()+"'",$rootScope.bsiReportParams.page=1,$scope.pageQuery(),$("#selected-batch span").html($("#"+batchId).next().find("span").text()),$("#selected-batch").removeClass("hide"),$("#to-sl-batch").addClass("hide"))},$scope.clearSlBatch=function(){$("#selected-batch").addClass("hide"),$("#to-sl-batch").removeClass("hide"),$("#batch-more").removeClass("disabled"),$("#batch-more").attr("disabled",!1),$("#batch-lists").find(".multisl-btns").addClass("hide"),$rootScope.bsiReportParams.batch=null,$rootScope.bsiReportParams.page=1,$scope.pageQuery()},$scope.batchMore=function(){$("#batch-lists").hasClass("show-more")?($("#batch-lists").removeClass("show-more"),$("#batch-more span").html("更多"),$("#batch-more i").removeClass("fa-chevron-up").addClass("fa-chevron-down")):($("#batch-lists").addClass("show-more"),$("#batch-more span").html("收起"),$("#batch-more i").removeClass("fa-chevron-down").addClass("fa-chevron-up"))},$scope.batchMultiselect=function(){$(".selector-line").removeClass("select-more"),$("#report-multibatch-search").addClass("disabled"),$("#report-multibatch-search").attr("disabled",!0),$(".selector-line .checkbox").removeClass("checkbox-ed").addClass("checkbox-un").addClass("hide"),$(".selector-line").find("[name='sl-confirm']").addClass("disabled"),$(".selector-line").find("[name='sl-confirm']").attr("disabled",!0),$(".selector-line").find(".multisl-btns").addClass("hide");var selectorline=$("#batch-sl");selectorline.addClass("select-more"),selectorline.find(".sl-val").addClass("show-more"),selectorline.find(".checkbox").removeClass("hide"),selectorline.find(".multisl-btns").removeClass("hide"),$("#batch-lists").addClass("show-more"),$("#batch-more span").html("收起"),$("#batch-more i").removeClass("fa-chevron-down").addClass("fa-chevron-up"),$("#batch-lists .checkbox").removeClass("checkbox-ed").addClass("checkbox-un"),$("#report-multibatch-search").addClass("disabled"),$("#report-multibatch-search").attr("disabled",!0),$("#batch-more").addClass("disabled"),$("#batch-more").attr("disabled",!0)},$scope.batchLists=function(batchId){$("#"+batchId).toggleClass("checkbox-un"),$("#"+batchId).toggleClass("checkbox-ed"),$("#batch-lists .checkbox-ed").size()>0?($("#report-multibatch-search").removeClass("disabled"),$("#report-multibatch-search").attr("disabled",!1)):($("#report-multibatch-search").addClass("disabled"),$("#report-multibatch-search").attr("disabled",!0))},$scope.reportMultibatchSearch=function(){var show_val=[];$("#batch-lists .checkbox-ed").each(function(){null==$rootScope.bsiReportParams.batch?$rootScope.bsiReportParams.batch="'"+$(this).next().text()+"'":$rootScope.bsiReportParams.batch+=",'"+$(this).next().text()+"'",show_val.push($(this).next().text())}),$scope.pageQuery(),$("#selected-batch span").html(show_val.toString()),$("#selected-batch").removeClass("hide"),$("#to-sl-batch").addClass("hide"),$("#batch-sl").removeClass("select-more"),$("#batch-lists").removeClass("show-more"),$("#batch-lists").find(".checkbox").addClass("hide"),$("#batch-more span").html("更多"),$("#batch-more i").removeClass("fa-chevron-up").addClass("fa-chevron-down")},$scope.resetBatchMultiSelect=function(){var selectorline=$("#batch-sl");selectorline.removeClass("select-more"),selectorline.find(".sl-val").removeClass("show-more"),selectorline.find(".checkbox").addClass("hide").addClass("checkbox-un").removeClass("checkbox-ed"),selectorline.find(".multisl-btns").addClass("hide"),$("#batch-more").hasClass("disabled")&&($("#batch-more").removeClass("disabled"),$("#batch-more").attr("disabled",!1),$("#batch-more span").html("更多"),$("#batch-more i").removeClass("fa-chevron-up").addClass("fa-chevron-down"))},$scope.reportPeriodSearch=function(periodId){$("#period-sl").hasClass("select-more")||($rootScope.bsiReportParams.period=$("#"+periodId).next().find("input").val(),$rootScope.bsiReportParams.page=1,$scope.pageQuery(),$("#selected-period span").html($("#"+periodId).next().find("span").text()),$("#selected-period").removeClass("hide"),$("#to-sl-period").addClass("hide"))},$scope.clearSlPeriod=function(){$("#selected-period").addClass("hide"),$("#to-sl-period").removeClass("hide"),$rootScope.bsiReportParams.period=null,$scope.pageQuery()},$scope.periodMultiselect=function(){$(".selector-line").removeClass("select-more"),$("#report-multibatch-search").addClass("disabled"),$("#report-multibatch-search").attr("disabled",!0),$(".selector-line .checkbox").removeClass("checkbox-ed").addClass("checkbox-un").addClass("hide"),$(".selector-line").find("[name='sl-confirm']").addClass("disabled"),$(".selector-line").find("[name='sl-confirm']").attr("disabled",!0),$(".selector-line").find(".multisl-btns").addClass("hide");var selectorline=$("#period-sl");selectorline.addClass("select-more"),selectorline.find(".sl-val").addClass("show-more"),selectorline.find(".checkbox").removeClass("hide"),selectorline.find(".multisl-btns").removeClass("hide")},$scope.periodLists=function(periodId){$("#"+periodId).toggleClass("checkbox-un"),$("#"+periodId).toggleClass("checkbox-ed"),$("#period-lists .checkbox-ed").size()>0?($("#report-multiperiod-search").removeClass("disabled"),$("#report-multiperiod-search").attr("disabled",!1)):($("#report-multiperiod-search").addClass("disabled"),$("#report-multiperiod-search").attr("disabled",!0))},$scope.reportMultiperiodSearch=function(){var show_val=[];$("#period-lists .checkbox-ed").each(function(){null==$rootScope.bsiReportParams.period?$rootScope.bsiReportParams.period=$(this).next().find("input[type='hidden']").val():$rootScope.bsiReportParams.period+=","+$(this).next().find("input[type='hidden']").val(),show_val.push($(this).next().find("span").html())}),$scope.pageQuery(),$("#selected-period span").html(show_val.toString()),$("#selected-period").removeClass("hide"),$("#to-sl-period").addClass("hide"),$("#period-sl").removeClass("select-more"),$("#period-sl").find(".checkbox").addClass("hide"),$("#period-sl").find(".multisl-btns").addClass("hide"),$("#period-lists .checkbox").removeClass("checkbox-ed").addClass("checkbox-un")},$scope.resetPeriodMultiSelect=function(){var selectorline=$("#period-sl");selectorline.removeClass("select-more"),selectorline.find(".sl-val").removeClass("show-more"),selectorline.find(".checkbox").addClass("hide").addClass("checkbox-un").removeClass("checkbox-ed"),selectorline.find(".multisl-btns").addClass("hide"),$("#batch-more").hasClass("disabled")&&($("#batch-more").removeClass("disabled"),$("#batch-more").attr("disabled",!1),$("#batch-more span").html("更多"),$("#batch-more i").removeClass("fa-chevron-up").addClass("fa-chevron-down"))},$scope.dateQuery=function(){$rootScope.bsiReportParams.beginDate=$("#report-begindate-search").val(),$rootScope.bsiReportParams.endDate=$("#report-enddate-search").val(),$scope.pageQuery()},$scope.distributeQuery=function(){var distribute=$("#distribute");distribute.find(".sl-judge-no").hasClass("hide")?(distribute.find(".sl-judge-no").removeClass("hide"),distribute.find(".sl-judge-yes").addClass("hide"),$rootScope.bsiReportParams.distributed=1):(distribute.find(".sl-judge-yes").removeClass("hide"),distribute.find(".sl-judge-no").addClass("hide"),$rootScope.bsiReportParams.distributed=0),$scope.pageQuery()},$scope.sortDate=function(){$rootScope.bsiReportParams.sort=0,$rootScope.bsiReportParams.sortDate="desc"==$rootScope.bsiReportParams.sortDate?"asc":"desc",$rootScope.sortIcon($rootScope.bsiReportParams),$scope.pageQuery()},$scope.sortBatch=function(){$rootScope.bsiReportParams.sort=1,$rootScope.bsiReportParams.sortBatch="desc"==$rootScope.bsiReportParams.sortBatch?"asc":"desc",$rootScope.sortIcon($rootScope.bsiReportParams),$scope.pageQuery()},$scope.sortName=function(){$rootScope.bsiReportParams.sort=2,$rootScope.bsiReportParams.sortName="desc"==$rootScope.bsiReportParams.sortName?"asc":"desc",$rootScope.sortIcon($rootScope.bsiReportParams),$scope.pageQuery()},$scope.sortPeriod=function(){$rootScope.bsiReportParams.sort=3,$rootScope.bsiReportParams.sortPeriod="desc"==$rootScope.bsiReportParams.sortPeriod?"asc":"desc",$rootScope.sortIcon($rootScope.bsiReportParams),$scope.pageQuery()},$scope.pageSizeQuery=function(){$rootScope.bsiReportParams.size=$("#page-size-sel").val(),$rootScope.bsiReportParams.page=1,$scope.pageQuery()},$scope.reRun=function(dataKey,appId,projectId){bsiService.reportReRun(dataKey,appId,projectId).success(function(){$scope.pageQuery()})},$scope.paginationBtn=function(currentPage){$rootScope.bsiReportParams.page=currentPage,$scope.pageQuery()},$scope.periodError=function(dataName){$("#run-error-data").html(dataName),$("#running-error-modal").modal("show")},$scope.pageQuery()}),celloudApp.controller("bsiDataController",function($scope,$rootScope,bsiService){$scope.params={page:1,size:$("#page-size-sel").val(),condition:null,sort:0,sortBatch:"asc",sortName:"asc",sortDate:"desc"},$scope.pageQuery=function(){bsiService.dataPageQuery($scope.params).success(function(dataMap){$scope.pageList=dataMap.pageList})},$scope.paginationBtn=function(currentPage){$scope.params.page=currentPage,$scope.pageQuery()},$scope.sortDate=function(){$scope.params.sort=0,$scope.params.sortDate="desc"==$scope.params.sortDate?"asc":"desc",$rootScope.sortIcon($scope.params),$scope.pageQuery()},$scope.sortBatch=function(){$scope.params.sort=1,$scope.params.sortBatch="desc"==$scope.params.sortBatch?"asc":"desc",$rootScope.sortIcon($scope.params),$scope.pageQuery()},$scope.sortName=function(){$scope.params.sort=2,$scope.params.sortName="desc"==$scope.params.sortName?"asc":"desc",$rootScope.sortIcon($scope.params),$scope.pageQuery()},$scope.pageSizeQuery=function(){$scope.params.size=$("#page-size-sel").val(),$scope.params.page=1,$scope.pageQuery()},$scope.pageQuery()})}();
