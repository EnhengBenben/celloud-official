...
!function(){celloudApp.controller("appListCtrl",["$scope","$routeParams","AppService",function($scope,$routeParams,AppService){function init(){AppService.classifysByPid($routeParams.cid).success(function(data){$scope.sclassifys=data,data.length>0&&($scope.nowCid=data[0].classifyId,$scope.nowCName=data[0].classifyName,$scope.currentPage=1,$scope.pageSize=10,getAppsByCid())})}function getAppsByCid(id){$scope.nowCid=void 0==id?$scope.nowCid:id,AppService.classifyAppsByCid($scope.nowCid,$scope.currentPage,$scope.pageSize).success(function(data){$scope.appPageInfo=data}),$("#app-classify-"+id).parent().parent().find(".active").removeClass("active"),$("#app-classify-"+id).addClass("active")}function pageQuery(currentPage,pageSize){$scope.currentPage=currentPage,$scope.pageSize=pageSize,getAppsByCid()}return $scope.pageQuery=pageQuery,$scope.getAppsByCid=getAppsByCid,init()}])}();
