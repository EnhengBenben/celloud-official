...
!function(){celloudApp.service("AppService",["$http",function($http){var self=this;self.classic=function(){return $http({method:"GET",url:"app/classic"})},self.recommend=function(){return $http({method:"GET",url:"app/recommend"})},self.classifyApps=function(){return $http({method:"GET",url:"app/classifys"})},self.classifys=function(){return $http({method:"GET",url:"classifys"})},self.classifysByPid=function(pid){return $http({method:"GET",url:"classifys",params:{pid:pid}})},self.updateAdd=function(appId){return $http({method:"PUT",url:"app/addOrRemoveApp",params:{appId:appId}})},self.classifyAppsByCid=function(cid,currentPage,pageSize){return $http({method:"GET",url:"app/classifys/"+cid,params:{currentPage:currentPage,pageSize:pageSize}})},self.appDetail=function(id){return $http({method:"GET",url:"app/"+id})},self.appComments=function(id,currentPage,pageSize){return $http({method:"GET",url:"appComments",params:{appId:id,currentPage:currentPage,pageSize:pageSize}})},self.updateComment=function(id,score,comment){return $http({method:"POST",url:"appComments",params:{appId:id,score:score,comment:comment}})}}])}();
!function(){celloudApp.controller("appClassifyCtrl",["$rootScope","AppService",function($rootScope,AppService){void 0==$rootScope.classifys&&AppService.classifys().success(function(data){$rootScope.classifys=data})}]),celloudApp.controller("appMainCtrl",["$rootScope","$scope","AppService",function($rootScope,$scope,AppService){function init(){classicApps(),recommendApps(),classifysApps(),appSwiperInit()}function appSwiperInit(){viewSwiper=new Swiper(".view .swiper-container",{initialSlide:0,observer:!0,observeParents:!0}),previewSwiper=new Swiper(".preview .swiper-container",{direction:"vertical",visibilityFullFit:!0,slidesPerView:"auto",onlyExternal:!0,observer:!0,observeParents:!0,slideToClickedSlide:!0,onClick:function(){viewSwiper.slideTo(previewSwiper.clickedIndex)}})}function appSwiperPre(){return 0==viewSwiper.activeIndex?void viewSwiper.slideTo(viewSwiper.slides.length-1,1e3):void viewSwiper.slidePrev()}function appSwiperNext(){return viewSwiper.activeIndex==viewSwiper.slides.length-1?void viewSwiper.slideTo(0,1e3):void viewSwiper.slideNext()}function classicApps(){AppService.classic().success(function(data){$scope.classicApps=data})}function recommendApps(){AppService.recommend().success(function(data){$scope.recommendApps=data})}function classifysApps(){AppService.classifyApps().success(function(data){$scope.classifysApps=data})}function refresh(){var appElement=document.querySelector("[ng-controller=sidebarController]"),element=angular.element(appElement),$scope=element.scope();$scope.refreshUserProduct()}function getApp(appId){AppService.updateAdd(appId).then(function(res){console.log("successCallback"+res.status+res.status==400),init(),refresh()},function(res){console.log("errorCallback"+res.status),400==res.status&&$.tips("您还没有此应用的使用权限，请通过右侧“联系我们”获取!")})}return $scope.getApp=getApp,$scope.appSwiperPre=appSwiperPre,$scope.appSwiperNext=appSwiperNext,$rootScope.appSwiperInit=appSwiperInit,init();var viewSwiper,previewSwiper}])}();
!function(){celloudApp.controller("appListCtrl",["$scope","$routeParams","AppService","$rootScope",function($scope,$routeParams,AppService,$rootScope){function init(){AppService.classifysByPid($routeParams.cid).success(function(data){$scope.sclassifys=data,$scope.prevClassifyId=$routeParams.cid,data.length>0&&($scope.nowCid=data[0].classifyId,$scope.nowCName=data[0].classifyName,$scope.currentPage=1,$scope.pageSize=10,console.log("cid = ",$routeParams.cid),console.log("rootClassifyId = ",$rootScope.rootClassifyId),console.log("nowCid = ",$scope.nowCid),console.log("appClassifyId = ",$rootScope.appClassifyId),console.log("nowCName = ",$scope.nowCName),console.log("appClassifyName = ",$rootScope.appClassifyName),void 0!=$rootScope.appClassifyId&&$rootScope.rootClassifyId==$routeParams.cid?($scope.nowCid=$rootScope.appClassifyId,$scope.nowCName=$rootScope.appClassifyName):($rootScope.appClassifyId=$scope.nowCid,$rootScope.appClassifyName=$scope.nowCName,$rootScope.rootClassifyId=$routeParams.cid),getAppsByCid())})}function getAppsByCid(id,name){void 0!=id&&($scope.nowCid=id,$scope.nowCName=name,$scope.currentPage=1,$scope.pageSize=10),AppService.classifyAppsByCid($scope.nowCid,$scope.currentPage,$scope.pageSize).success(function(data){$scope.appPageInfo=data}),$("#app-classify-"+id).parent().parent().find(".active").removeClass("active"),$("#app-classify-"+id).addClass("active")}function pageQuery(currentPage,pageSize){$scope.currentPage=currentPage,$scope.pageSize=pageSize,getAppsByCid()}function refresh(){var appElement=document.querySelector("[ng-controller=sidebarController]"),element=angular.element(appElement),$scope=element.scope();$scope.refreshUserProduct()}function getApp(appId){AppService.updateAdd(appId).then(function(res){init(),refresh()},function(res){console.log("errorCallback"+res.status),400==res.status&&$.tips("您还没有此应用的使用权限，请通过右侧“联系我们”获取!")})}return $scope.getApp=getApp,$scope.pageQuery=pageQuery,$scope.getAppsByCid=getAppsByCid,init()}])}();
!function(){celloudApp.controller("appDetailCtrl",["$scope","$routeParams","AppService","$rootScope",function($scope,$routeParams,AppService,$rootScope){function init(){AppService.appDetail($routeParams.id).success(function(data){if($(document).scrollTop(0),$scope.appInfos=data,void 0!=$scope.appInfos.userComment){$scope.userComment=$scope.appInfos.userComment.comment;for(var i=1;i<=$scope.appInfos.userComment.score;i++)$("#score-"+i).attr("class","user-click score-icon score-full");$scope.userScore=$scope.appInfos.userComment.score}$rootScope.appClassifyId=$scope.appInfos.classifyId,$rootScope.appClassifyName=$scope.appInfos.classifys,$scope.countScore5=void 0==data.countScore[5]?0:data.countScore[5].count,$scope.countScore4=void 0==data.countScore[4]?0:data.countScore[4].count,$scope.countScore3=void 0==data.countScore[3]?0:data.countScore[3].count,$scope.countScore2=void 0==data.countScore[2]?0:data.countScore[2].count,$scope.countScore1=void 0==data.countScore[1]?0:data.countScore[1].count,$scope.scoreStype5={width:$scope.countScore5/data.totalCount*100+"%"},$scope.scoreStype4={width:$scope.countScore4/data.totalCount*100+"%"},$scope.scoreStype3={width:$scope.countScore3/data.totalCount*100+"%"},$scope.scoreStype2={width:$scope.countScore2/data.totalCount*100+"%"},$scope.scoreStype1={width:$scope.countScore1/data.totalCount*100+"%"},$scope.screenShow=data.screens[0]}),$scope.currentPage=1,$scope.pageSize=10,appComments()}function appComments(){AppService.appComments($routeParams.id,$scope.currentPage,$scope.pageSize).success(function(data){$scope.appComments=data})}function pageQuery(currentPage,pageSize){$scope.currentPage=currentPage,$scope.pageSize=pageSize,appComments()}function updateScore(score){$(".user-click").attr("class","user-click score-icon score-gray-null");for(var i=1;i<=score;i++)$("#score-"+i).attr("class","user-click score-icon score-full");$scope.userScore=score}function updateComment(){return $scope.userScore?!$scope.userComment||$scope.userComment.length<5?void $.tips("评论不能少于五个字符！"):void AppService.updateComment($routeParams.id,$scope.userScore,$scope.userComment).success(function(){$.tips("评论成功"),init()}):void $.tips("评分不能为空！")}function appScreenShow(img,index){$scope.screenShow=img,$("#screens-ul .active").removeClass("active"),$("#screen-"+index).addClass("active")}function refresh(){var appElement=document.querySelector("[ng-controller=sidebarController]"),element=angular.element(appElement),$scope=element.scope();$scope.refreshUserProduct()}function getApp(appId){AppService.updateAdd(appId).then(function(res){console.log("successCallback"+res.status+res.status==400),init(),refresh()},function(res){console.log("errorCallback"+res.status),400==res.status&&$.tips("您还没有此应用的使用权限，请通过右侧“联系我们”获取!")})}return $scope.getApp=getApp,$scope.pageQuery=pageQuery,$scope.updateScore=updateScore,$scope.updateComment=updateComment,$scope.appScreenShow=appScreenShow,$rootScope.rootClassifyId=$routeParams.classifyId,init()}])}();
...
!function(){celloudApp.controller("dataListController",function($scope,$rootScope,$location,$routeParams,$translate,runService,uploadService){$scope.pageQuery=function(page,pageSize){$.dataManager.options.condition=$scope.dataCondition,$.dataManager.options.page=page,$.dataManager.options.pageSize=pageSize,runService.pageList().success(function(response){$scope.dataList=response}),$.dataManager.refreshDataList("clean")},$.dataManager.options.sort=0,$scope.sortQuery=function(column,order){console.log(column,order),"anotherName"==column?($scope.column="anotherName",$.dataManager.options.sort=3,"asc"==order?($scope.order="desc",$.dataManager.options.sortAnotherName="desc"):($scope.order="asc",$.dataManager.options.sortAnotherName="asc")):"run"==column&&($scope.column="run",$.dataManager.options.sort=4,"asc"==order?($scope.order="desc",$.dataManager.options.sortRun="desc"):($scope.order="asc",$.dataManager.options.sortRun="asc")),runService.pageList().success(function(response){$scope.dataList=response})},$scope.pageQuery($.dataManager.options.page,$.dataManager.options.pageSize),$scope.conditionList=function(){$scope.pageQuery(1,$.dataManager.options.pageSize)},$scope.doSearch=function($event){13==$event.keyCode&&$scope.conditionList()},$scope.checkNum=function(){var checkedIds=$.dataManager.options.checkedIds;if(0==checkedIds.length)return $.tips($translate.instant("PROMPT.AT_LEAST_ONE"),$translate.instant("PROMPT.PROMPT")),!1;var maxNum=$.dataManager.options.dataMax;return!(checkedIds.length>maxNum)||($.tips($translate.instant("PROMPT.ARCHIVE_MAX",{maxNum:maxNum}),$translate.instant("PROMPT.PROMPT")),!1)},$scope.runWithProject=function(){uploadService.checkRole().success(function(data){if("0"==data)return void $.alert($translate.instant("ALERT.RUN_SUCC"));if($scope.checkNum())return 0!=$.dataManager.options.isRun?void $.tips($translate.instant("PROMPT.SELECT_NOT_RUN"),$translate.instant("PROMPT.PROMPT")):0!=$.dataManager.options.isTag?void $.tips($translate.instant("PROMPT.SELECT_NO_PRODUCT"),$translate.instant("PROMPT.PROMPT")):0!=$.dataManager.options.isBSI?void $.tips($translate.instant("PROMPT.SELECT_NOT_BACTIVE"),$translate.instant("PROMPT.PROMPT")):0!=$.dataManager.options.isRocky?void $.tips($translate.instant("PROMPT.SELECT_NOT_ROCKY"),$translate.instant("PROMPT.PROMPT")):void(0!=$.dataManager.options.isPair?(window.modalConfirmBtn=$translate.instant("BUTTON.CONFIRM"),window.modalCancelBtn=$translate.instant("BUTTON.CANCEL"),$.confirm($translate.instant("CONFIRM.DATA_PAIRED"),$translate.instant("CONFIRM.CONFIRM"),function(){$scope.run()})):$scope.run())})};var opts={lines:13,length:28,width:14,radius:42,corners:1,rotate:0,direction:1,color:"#000",speed:1,trail:60,shadow:!1,hwaccel:!1,className:"spinner",zIndex:2e9,top:"auto",left:"50%"};$scope.run=function(){var checkedIds=$.dataManager.options.checkedIds;if(checkedIds.length!=$.dataManager.options.validIds)return $rootScope.errorInfo="数据选择异常，请刷新页面后重新选择！",void $("#tips-modal").modal("show");spinner=new Spinner(opts);var target=document.getElementById("_data_table");spinner.spin(target),runService.run().success(function(response){response.success?($.alert($translate.instant("ALERT.RUN_SUCC")),$scope.pageQuery($.dataManager.options.page,$.dataManager.options.pageSize),$.dataManager.refreshDataList()):$scope.message=response.message,spinner.stop()})},$scope.deleteData=function(){uploadService.checkRole().success(function(data){if("0"==data)$.tips($translate.instant("PROMPT.TEST_USER_NO_ARCHIVE"),$translate.instant("PROMPT.PROMPT"));else{if(!$scope.checkNum())return;window.modalConfirmBtn=$translate.instant("BUTTON.CONFIRM"),window.modalCancelBtn=$translate.instant("BUTTON.CANCEL"),$.confirm($translate.instant("CONFIRM.ARCHIVE_DATA"),$translate.instant("CONFIRM.CONFIRM"),function(){runService.deleteData().success(function(response){response.success&&($scope.conditionList(),$.dataManager.refreshDataList()),$.alert($translate.instant("ALERT.ARCHIVE_SUCC"))})})}})},$scope.toEditData=function(fileId){uploadService.checkRole().success(function(data){"0"==data?$.tips($translate.instant("PROMPT.TEST_USER_NO_MODIFIED"),$translate.instant("PROMPT.PROMPT")):($scope.updateState=!1,runService.toEditData(fileId).success(function(response){for($scope.dataFile=response.file,$scope.appList=response.appList,i=0;i<$scope.appList.length;i++)if($.trim($scope.appList[i].tagName)==$.trim($scope.dataFile.tagName)){$scope.appSelected=$scope.appList[i];break}}),$("#data-detail-modal").modal("show"))})},$scope.submitEditData=function(){var alias=$scope.dataFile.anotherName;if(null!=alias&&alias.length>50)return void $.tips($translate.instant("PROMPT.FILE_ALIAS_MAX_CHAR",{maxNum:50}),$translate.instant("PROMPT.PROMPT"));var batch=$scope.dataFile.batch;return null!=batch&&batch.length>50?void $.tips($translate.instant("PROMPT.DATA_LABEL_MAX_CHAR",{maxNum:50}),$translate.instant("PROMPT.PROMPT")):($scope.updateState=!1,$scope.dataFile.tagName=void 0==$scope.appSelected?"":$scope.appSelected.tagName,void runService.submitEditData($scope.dataFile).success(function(response){response.success&&($scope.pageQuery($.dataManager.options.page,$.dataManager.options.pageSize),$.dataManager.refreshDataList(),$("#data-detail-modal").modal("hide")),$.alert($translate.instant("ALERT.MODIFIED_SUCC"))}))}})}();
