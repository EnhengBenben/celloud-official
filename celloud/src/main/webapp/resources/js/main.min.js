...
!function(){celloudApp.service("companyService",function($resource,$http){this.pageQueryUser=function(currentPage,pageSize){return pageSize=pageSize||10,$http({method:"GET",url:"company/userInfo",params:{currentPage:currentPage,pageSize:pageSize}})}})}();
!function(){celloudApp.controller("companyUserController",function($scope,companyService){$scope.pageQuery=function(currentPage,pageSize){companyService.pageQueryUser(currentPage,pageSize).success(function(userList){$scope.userList=userList})},$scope.pageQuery(1,10)})}();