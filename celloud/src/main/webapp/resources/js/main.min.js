...
!function(){celloudApp.controller("appClassifyCtrl",["$rootScope","AppService",function($rootScope,AppService){void 0==$rootScope.classifys&&AppService.classifys().success(function(data){$rootScope.classifys=data})}]),celloudApp.controller("appMainCtrl",["$scope","AppService",function($scope,AppService){$scope.getClassicApps=function(){AppService.classic().success(function(data){$scope.classicApps=data})},$scope.getRecommendApps=function(){AppService.recommend().success(function(data){$scope.recommendApps=data})},$scope.getClassifysApps=function(){AppService.classifyApps().success(function(data){$scope.classifysApps=data})},$scope.getApp=function(appId){AppService.updateAdd(appId).then(function(res){console.log("successCallback"+res.status+res.status==400),$scope.getClassifysApps()},function(res){console.log("errorCallback"+res.status),400==res.status&&$.tips("您还没有此应用的使用权限，请通过右侧“联系我们”获取!")})},$scope.getClassicApps(),$scope.getRecommendApps(),$scope.getClassifysApps()}])}();
...
!function(){celloudApp.controller("appDetailCtrl",["$scope","$routeParams","AppService",function($scope,$routeParams,AppService){function init(){AppService.appDetail($routeParams.id).success(function(data){$scope.appInfos=data,console.log(data.countScore),$scope.countScore5=void 0==data.countScore[5]?0:data.countScore[5].count,$scope.countScore4=void 0==data.countScore[4]?0:data.countScore[4].count,$scope.countScore3=void 0==data.countScore[3]?0:data.countScore[3].count,$scope.countScore2=void 0==data.countScore[2]?0:data.countScore[2].count,$scope.countScore1=void 0==data.countScore[1]?0:data.countScore[1].count,$scope.scoreStype5={width:$scope.countScore5/data.totalCount*100+"%"},$scope.scoreStype4={width:$scope.countScore4/data.totalCount*100+"%"},$scope.scoreStype3={width:$scope.countScore3/data.totalCount*100+"%"},$scope.scoreStype2={width:$scope.countScore2/data.totalCount*100+"%"},$scope.scoreStype1={width:$scope.countScore1/data.totalCount*100+"%"}}),$scope.currentPage=1,$scope.pageSize=10,appComments()}function appComments(){AppService.appComments($routeParams.id,$scope.currentPage,$scope.pageSize).success(function(data){$scope.appComments=data})}function pageQuery(currentPage,pageSize){$scope.currentPage=currentPage,$scope.pageSize=pageSize,appComments()}function updateScore(score){for(var i=1;i<score;i++)$("score-"+i).attr("class","score-icon score-full");$scope.userScore=score}function updateComment(){AppService.updateComment($routeParams.id,$scope.userScore,$scope.userComment).success(function(){$.tips("评论成功")})}return $scope.pageQuery=pageQuery,$scope.updateScore=updateScore,$scope.updateComment=updateComment,init()}])}();
