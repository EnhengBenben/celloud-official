...
public class FeedbackConstants {
     ...
     * @return
     */
    public static String getAttachment(String nameWithSuffix,Integer userid) {
        int index = nameWithSuffix.indexOf(".");
        ObjectId id = new ObjectId(nameWithSuffix.substring(0, index));
        return getAttachmentPath(id.getDate(),userid) + File.separator + nameWithSuffix;
    }
     ...
     * @return
     */
    public static String getAttachmentPath(Date date,Integer userid) {
        if (attachmentPath == null) {
            attachmentPath = getProperty("feedback_attatchment_path");
        }
        return attachmentPath + File.separator + userid + File.separator
