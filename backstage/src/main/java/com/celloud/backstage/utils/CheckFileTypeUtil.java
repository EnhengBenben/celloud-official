...
import com.celloud.backstage.constants.FileTitleType;
    ...
    public int checkFileType(String fileName) {
        ...
        if (checkTileType(FileTitleType.ABI, fileName)
            ...
            fileType = FileFormat.FENGTU;
                ...
                && ((".zip".equals(extName.toLowerCase()) && (checkTileType(FileTitleType.ZIP, fileName)))
                        || (".gz".equals(extName.toLowerCase()) && (checkTileType(FileTitleType.GZ, fileName))))) {
            // 文件为压缩文件
            fileType = FileFormat.YASUO;
        } else if (checkTileType(FileTitleType.BAM, fileName) && extName.equals(".bam")) {
            // bam格式文件
            fileType = FileFormat.BAM;
        } else if (checkTileType(FileTitleType.FASTA, fileName) && (checkFasta(fileName)
